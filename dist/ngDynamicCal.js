...
dynamicCal.directive('calCalendar', ['$document', 'calEventHandler', 'calDayObject', '$timeout', function ($document, calEventHandler, calDayObject, $timeout) {
    return {
        ...
        link: function (scope, elem, attrs, controller) {
            ...
            //scope.view = {
            //    title: "",
            //    durration: config.defaultDurration != null ? config.defaultDurration : 'week',
            //    type: config.defaultType != null ? config.defaultType : 'schedule',
            //    date: new Date(today),
...
dynamicCal.controller('calCalendarCtrl', ["$scope", function ($scope) {
...
angular.module('dynamicCal').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('calDay.html',
    ...
    "\n" +
    "\t\t<div ng-repeat=\"i in [] | range:calendar.viewStart:calendar.viewEnd:.5\" ng-class=\"{ 'cal-hourMark' : i % 1 == 0, 'cal-halfHourMark' : i % 1 != 0 }\" style=\"height:{{calendar.cellHeight + 'px'}};\"></div><!--ng-class=\"i % 1 == 0 ? 'cal-hourMark' : 'cal-halfHourMark'\"></div>-->\r" +
    "\n" +
    "\t</div>\r" +
    "\n" +
  ...
  $templateCache.put('calWeek.html',
    ...
    "\t\t\t\t<div class=\"cal-calendar-grid\">\r" +
    "\n" +
    "\t\t\t\t\t<div ng-repeat=\"i in [] | range:start:end:.5\" class=\"{{i%1==0?'cal-hourMark':'cal-halfHourMark'}}\" style=\"height:{{calendar.cellHeight + 'px'}};\">\r" +
    "\n" +
